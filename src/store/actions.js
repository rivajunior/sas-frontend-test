...
export default {
  ...
  loadQuestions({ commit }, { categoryId, difficult = '', amount = 10, type = 'multiple' }) {
    commit(mutationsTypes.START_LOADING)

    return api
      .get(
        `api.php?amount=${amount}&category=${categoryId}&type=${type}&difficulty=${difficult}&encode=base64`
      )
      .then(response => {
